...
import InstallView from "./InstallView.jsx";
...
function buildClusterVarsYaml(classified, cc, nodeNetworks) {
  ...
  if (ntpList.length > 0) { lines.push("ntp_servers:"); lines.push(yamlList(ntpList)); }
  if (dnsList.length > 0) { lines.push("dns_servers:"); lines.push(yamlList(dnsList)); }
...
export default function App() {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      const yaml = buildClusterVarsYaml(classified, clusterConfig, nodeNetworks);
  ...
  const handleInstall = useCallback(async () => {
    if (!exported || !clusterConfig.cluster_name) return;
    setInstalling(true);
    try {
      await api.triggerInstall(clusterConfig.cluster_name, exportedYaml);
      setInstallStatus("running");
      setActiveTab("install");
    } catch (err) {
      setAlert({ variant: "danger", title: "Failed to trigger install.", body: err.message });
    } finally {
      setInstalling(false);
    }
  }, [exported, clusterConfig.cluster_name, exportedYaml]);

  // Poll install status + log while running
  useEffect(() => {
    const name = clusterConfig.cluster_name;
    if (!name || installStatus !== "running") return;
    const id = setInterval(async () => {
      try {
        const [st, lg] = await Promise.all([
          api.getInstallStatus(name),
          api.getInstallLog(name),
        ]);
        setInstallStatus(st.status);
        setInstallLog(lg.lines || []);
        if (st.status !== "running") clearInterval(id);
      } catch { /* non-fatal */ }
    }, POLL_MS);
    return () => clearInterval(id);
  }, [installStatus, clusterConfig.cluster_name]);
    ...
    <div>
      ...
      <div style={{ padding: "1.5rem" }}>
        ...
        >
          ...
          <Tab
            eventKey="install"
            title={<TabTitleText>Install</TabTitleText>}
          >
            <InstallView
              exported={exported}
